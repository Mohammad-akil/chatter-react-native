...
import { api } from '~/api';
export const uploadImage = async (selectedChannelId: string, type: 'avatar' | 'banner') => {
  ...
  const data = await api.channel.uploadAvatarOrBanner(selectedChannelId, formData);
  if (!data.success) {
    Toast.show({
      type: 'error',
      text1: data.error,
